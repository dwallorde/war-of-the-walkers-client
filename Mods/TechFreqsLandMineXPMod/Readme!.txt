...
- updated code for tooltip error fix on new game version
- still server side compatible if installed on both


Key Features:
Current Status:
✅ Basic XP system shines at 150 XP per kill
✅ Shared XP is still TBA it works but ONLy if Client has it installed on their PC 
✅ Enemy animals like zombie dogs and bears grant XP, no glitches
✅ Tooltip pops up for local players— “Gained 150 XP from land mine kill!”
✅ Improve logging and feedback so each player gets clear confirmation of earned XP in LOGs via F1 without so much log clutter
✅ Fixed XP inconsistencies and anything else that needs adjustments...


If a single player is near the spike traps, they receive XP (100 XP per kill).
If multiple players are nearby, the mod attempts to determine who to send the XP to,but if not in distance of the mines, no xp will be given 
...
Previous Changelog:
As for V2:
In this update, same features as previous ONLY difference is that it detects just landmines, as previous it was detecting spike kills in the logs for some odd reason ,additional debugging logs thats useful for entity id, time and xyz coordinates of said mines around the player and kills in game, improved config json not just for radius but now adjustable detection proximity radius and xp amount.
...
cross posted onto 7daystodiemods, checkout TechFreq on nexusmods for further updates on their and private dms for mod suggestions or feedback!


As for V1:
allows players to gain XP from enemy zombies and most hostile animals killed by land mines. The XP distribution is proximity-based, meaning that if a player is near the traps when a kill occurs, they will receive XP, inconsistent xp sharing however configurable json file.
